...
export function VehiclesPanel() {
  ...
  const dragReorder = useCommand({
    execute: async (ids: string[]) => {
      const results = await Promise.all(ids.map((id, index) => db.from("vehicle_brands").update({ sort_order: index + 1 }).eq("id", id)));
      const failed = results.find((result: { error?: Error }) => result.error);
      if (failed?.error) throw failed.error;
    },
    onSuccess: () => { refresh(); toast.success("Brand order saved."); },
    onError: () => toast.error("Could not reorder brands."),
  });
  const visible = brands.filter(brand => brand.name.toLowerCase().includes(query.trim().toLowerCase()));

  return <div className="space-y-5"><section className="rounded-2xl border border-border bg-card p-4"><h2 className="font-bold">Vehicle brands</h2><p className="mt-1 text-sm text-muted-foreground">Choose a vehicle card, then drag brands to set their order in the pickup search.</p></section>{isLoading ? <div className="flex justify-center py-16"><Loader2 className="size-6 animate-spin text-primary" /></div> : <><section className="rounded-2xl border border-border bg-card p-4"><Select value={categoryId} onValueChange={id => { setCategoryId(id); setQuery(""); }}><SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Choose vehicle category" /></SelectTrigger><SelectContent>{categories.map(category => <SelectItem key={category.id} value={category.id}>{category.name}</SelectItem>)}</SelectContent></Select></section><section className="rounded-2xl border border-border bg-card p-4"><div className="flex items-center justify-between gap-3"><h2 className="font-bold">{selectedCategory ? `${selectedCategory.name} brands` : "Brands"}</h2>{categoryId && <span className="text-xs font-semibold text-muted-foreground">{brands.length} total</span>}</div><div className="mt-4 flex gap-2"><Input disabled={!categoryId} value={brandName} placeholder={categoryId ? "e.g. Maruti Suzuki" : "Choose a vehicle category first"} onChange={event => setBrandName(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && brandName.trim()) add.run(brandName); }} /><Button size="icon" variant="hero" disabled={!categoryId || !brandName.trim() || add.isPending} onClick={() => add.run(brandName)}>{add.isPending ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />}</Button></div>{brands.length > 6 && <div className="relative mt-3"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search brands" className="pl-9" /></div>}<div className="mt-3 space-y-2">{visible.length ? <DraggableOrder items={visible} disabled={Boolean(query)} onReorder={ids => dragReorder.run(ids)} className="space-y-2" renderItem={(brand) => { const index = brands.findIndex(item => item.id === brand.id); return <BrandRow brand={brand} position={index + 1} canMoveUp={index > 0} canMoveDown={index < brands.length - 1} onMove={direction => reorder.run({ index, direction })} onChanged={refresh} />; }} /> : <p className="py-6 text-center text-sm text-muted-foreground">{categoryId ? "No brands yet — add the first one above." : "Choose a vehicle category first."}</p>}</div></section></>}</div>;
}
