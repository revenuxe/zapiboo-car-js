...
export default function OrdersPage() {
  ...
  const { data, isLoading, mutate } = useSWR(user ? ["customer-pickups", user.id] : null, async () => {
    const { data, error } = await supabase.from("leads").select("id,pickup_id,status,vehicle_type,items,brand_name,model_name,registration_number,preferred_date,slot,address,locality,pincode,created_at").eq("user_id", user!.id).order("created_at", { ascending: false });
